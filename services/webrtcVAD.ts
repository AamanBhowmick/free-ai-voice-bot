...
const DEFAULT_CONFIG: VADConfig = {
  frameDurationMs: 30,
  sampleRate: 8000,
  aggressiveness: 2, // Good balance for telephone audio
};
 ...
 * 
 * Voiced speech has moderate ZCR; unvoiced noise has very high ZCR;
 * silence has near-zero ZCR.
 */
...
function isFrameSpeech(
  ...
  // Frame is speech if:
  // 1. Energy is above threshold (not silence)
  // 2. ZCR is within the human speech range (not white noise / static)
 ...
 *
 * @param pcmBuffer - Raw PCM Int16LE audio at 8kHz
 * @param config - Optional VAD configuration
 * @returns Object with detection result and statistics
 */
 ...
 * Convenience wrapper around detectVoice().
 */
